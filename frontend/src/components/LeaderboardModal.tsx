...
import {
  ...
  getIdentity, createIdentity, fetchToday, fetchAlltime,
...
export function LeaderboardModal({ isOpen, onClose, onIdentityCreated, usernameError, t }: LeaderboardModalProps) {
  ...
  useEffect(() => {
    if (!isOpen) return
    setLoading(true)
    Promise.all([fetchToday(), fetchAlltime()])
      .then(([td, at]) => { setToday(td); setAlltime(at) })
  ...
  const handleCreate = () => {
    if (!USERNAME_RE.test(nameInput)) return
    onIdentityCreated(createIdentity(nameInput))
  }
      ...
      <div className="card p-6 max-w-md w-full mx-4 max-h-[80vh] flex flex-col">
          ...
          <div className="mb-4 p-4 bg-blue-50 dark:bg-gray-700 rounded-lg">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                disabled={!USERNAME_RE.test(nameInput)}
        ...
        <div className="flex gap-2 mb-4">
          <button
            ...
            className={tab === 'today' ? 'btn-primary flex-1' : 'btn-secondary flex-1'}
          >
            {t.today}
          </button>
          <button
            onClick={() => setTab('alltime')}
            className={tab === 'alltime' ? 'btn-primary flex-1' : 'btn-secondary flex-1'}
          >
            {t.all_time}
          </button>
        </div>
              ...
              <tbody>
                {rows.map((entry, idx) => {
                  ...
                  return (
                    <tr
                      key={entry.username}
                      className={`border-b border-gray-100 dark:border-gray-700 ${isMe ? 'bg-blue-50 dark:bg-gray-700 font-semibold' : ''}`}
                    >
                      ...
                      <td className="py-2 text-right text-gray-900 dark:text-white">
                        {tab === 'today'
                          ? `${(entry as TodayEntry).score} ${t.points_abbr}`
                          : `${(entry as AlltimeEntry).total_score} ${t.points_abbr}`}
                      </td>
